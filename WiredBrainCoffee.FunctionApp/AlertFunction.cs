...
namespace WiredBrainCoffee.FunctionApp
{
    public static class AlertFunction
    {
        ...
        public static void Run(
            [EventHubTrigger("wiredbraincoffeeeh",
                Connection = "InputEventHubConnectionString",
                ConsumerGroup = "wired_brain_coffee_alert_function")]
            string[] inputMessages,
            [EventHub("wiredbraincoffeealerteh", Connection = "OutputEventHubConnectionString")]
            ...
            TraceWriter log)
        {
            foreach (var inputMessage in inputMessages)
            {
                ...
                if (data.SensorType == "CounterCappucino" && data.SensorValue % 10 == 0)
                {
                    outputMessages.Add(JsonConvert.SerializeObject(data));
                }
            }
        }
    }
